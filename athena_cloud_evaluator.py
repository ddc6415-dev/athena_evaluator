...
if is_ready:
    ...
    with st.spinner("Processing evaluation via Gemini 2.5 Flash..."):
        try:
            ...
            # ENHANCED PROMPT FOR MONOSPACE VERTICAL ALIGNMENT AND STRICT NEWLINES
            fence = "```"
            payload = {
                "contents": [{"parts": [{"text": (
                    f"You are the Author node of the Athena-1 system.\n\n"
                    f"Knowledge is information sufficiently accurate for repeated use.\n\n"
                    f"TASK 1 (Categorization): On the very first line, output ONLY the Operational Category. Example: [LEVEL: HYPOTHESIS / SPECULATIVE INFORMATION].\n\n"
                    f"TASK 2 (Graphic Display): On the very next line, output EXACTLY this header and nothing else: -- HEXAXIAL METRIC DISTRIBUTION --\n\n"
                    f"TASK 3 (Metrics): You MUST list the six axes inside a markdown code block ({fence}text). You MUST place EXACTLY ONE metric per line. Do NOT combine metrics on the same line. Pad the labels with spaces so the left brackets align perfectly. Format EXACTLY like this:\n"
                    f"{fence}text\n"
                    f"Repeatability:            [██████░░░░]\n"
                    ...
                    f"TASK 5 (Advertising): Conclude with an [ADVERTISING_INTRUSION_LOG]. If no ads are present in the payload, state 'No commercial advertisements detected.' Do NOT invent or generate an advertisement.\n\n"
                    f"--- PAYLOAD ---\n{extracted_text}"
                )}]}]
            }
